        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # determine if a letter key was pressed
                    ...
                    mode = 'erase'
                    if points:   
                        strokes.append((points.copy(), mode, radius)) # Save previous stroke before changing mode
            ...
            if event.type == pygame.MOUSEMOTION:
                if event.buttons[0]:   # Left mouse button is held
                    # Save completed freehand stroke
                    if drawing_mode == 1:
                        position = event.pos
...
def drawfig(screen, index, start, end, width, color_mode, draw_mode):
    x1, y1 = start
    x2, y2 = end
    # Distance formula used to calculate circle radius
